...
K_params = np.array([
    [12792.00000000, 0.00000000e+00, 400.000000000],
    [0.00000000e+00, 12792.00000000, 533.000000000],
    [0.00000000e+00, 0.00000000e+00, 1.0000000e+00]
])

# K_params = np.array([
#     [1972.47338e+00, 0.00000000e+00, 722.00547e+00],
#     [0.00000000e+00, 1978.22943e+00, 955.12887e+00],
#     [0.00000000e+00, 0.00000000e+00, 1.0000000e+00]
# ])
# K_params =np.array ([[1484.00168,    0.      , 929.48773],
...
def compute_orientation(point,img):
    ...
    for peak_idx in local_peaks:
        ...
        if local_peak_value >= 0.9 * dominant_orientation:            
    ...
    for kp_group in keypoints:
        for kp in kp_group:
            ...
            if flag:
                ...
                plt.plot(y, x, 'ro', markersize=4)  # 'ro' for red circles
            else:
                ...
                plt.arrow(y, x, arrow_y - y, arrow_x - x, color='blue', head_width=2, head_length=2, width=0.5)
...
# Shebl 10/22
def match_descriptors(descriptors1, descriptors2,kp1,kp2):
    ...
    for best, secondBest in matches:
        if best.distance < 0.9 * secondBest.distance:
            
            pt1=(kp1[best.queryIdx][0],kp1[best.queryIdx][1])
            
            
            pt2=(kp2[best.trainIdx][0],kp2[best.trainIdx][1])
            

            if pt1 in all_pts1 or  pt2 in all_pts2:
                continue
...
def draw_matches(img1, img2, kp1, kp2, good_matches, min_matches=10):
    ...
    if len(good_matches) >= 1:
        ...
        for m in good_matches:
            ...
            # print("first point")
            # print((int(kp1[m.queryIdx][1]),int(kp1[m.queryIdx][0])))
            # print("second point")
            # print((int(kp2[m.trainIdx][1] ), int(kp2[m.trainIdx][0])))
...
def apply_mask(image, mask):
    # Ensure mask is binary (0s and 255s) and matches the image size
    mask = cv2.resize(mask, (image.shape[1], image.shape[0]))
    mask = mask / 255 if mask.max() > 1 else mask  # Normalize if needed
    # Apply the mask by bitwise AND operation
...
def get_matches(img1_path="2.jpg",folder_1="new_resized2",img2_path="3.jpg",folder_2="new_resized2"):
    ...
    img1 = cv2.imread(f"{folder_1}/{img1_path}",0)
    img2 = cv2.imread(f"{folder_2}/{img2_path}",0)  
    ...
    if os.path.exists(f"key_points_saved/{folder_1}/{img1_path}.pkl") and  os.path.exists(f"descriptors_saved/{folder_1}/{img1_path}.pkl")  :

        
        key_points1=load_keypoints(f"key_points_saved/{folder_1}/{img1_path}.pkl")
        descriptors1=load_keypoints(f"descriptors_saved/{folder_1}/{img1_path}.pkl")
        ...
        save_keypoints(key_points1, f"key_points_saved/{folder_1}/{img1_path}.pkl")      
        descriptors1=generate_descriptors(key_points1,all_imgs1)
        save_keypoints(descriptors1,f"descriptors_saved/{folder_1}/{img1_path}.pkl")


    if os.path.exists(f"key_points_saved/{folder_2}/{img2_path}.pkl") and  os.path.exists(f"descriptors_saved/{folder_2}/{img2_path}.pkl")  :

        key_points2=load_keypoints(f"key_points_saved/{folder_2}/{img2_path}.pkl")
        descriptors2=load_keypoints(f"descriptors_saved/{folder_2}/{img2_path}.pkl")
        ...
        save_keypoints(key_points2, f"key_points_saved/{folder_2}/{img2_path}.pkl")      
        descriptors2=generate_descriptors(key_points2,all_imgs2)
        save_keypoints(descriptors2,f"descriptors_saved/{folder_2}/{img2_path}.pkl")
        

    
    matches=match_descriptors(descriptors1,descriptors2,key_points1,key_points2)
    
    

    
    # img1_8bit = convert_to_8bit( cv2.undistort(cv2.imread(f"{folder_1}/{img1_path}",0),K_params,distortion1))
    # img2_8bit = convert_to_8bit( cv2.undistort(cv2.imread(f"{folder_2}/{img2_path}",0),K_params,distortion1))

    # img1_8bit=cv2.undistort(img1,K_params,distortion1)
    # img2_8bit=cv2.undistort(img1,K_params,distortion1)
    # print("image is undistorted")
    # matches=matches[0:5]
...
def undistort_image(image, K, distCoeffs):
    undistorted_image = cv2.undistort(image, K, distCoeffs)
    return undistorted_image


def get_descriptors_and_keypoints(img_path,folder):
    
    print("we are here")
    print(img_path," ",folder)
    if os.path.exists(f"key_points_saved/{folder}/{img_path}.pkl"):
        print("it exists")
        
        return load_keypoints(f"descriptors_saved/{folder}/{img_path}.pkl"),load_keypoints(f"key_points_saved/{folder}/{img_path}.pkl")

    img1 = cv2.imread(f"{folder}/{img_path}",0)
    # img1 = undistort_image(img1, K_params, distortion1)
    ...
    save_keypoints(key_points1,f"key_points_saved/{folder}/{img_path}.pkl")
    save_keypoints(descriptors1,f"descriptors_saved/{folder}/{img_path}.pkl")
    # plot_keypoints_on_image(img1,key_points1,True)
    return descriptors1,key_points1


def get_descriptors_and_keypoints2(img_path,folder):
    
    if os.path.exists(f"descriptors_saved/{folder}/{img_path}.pkl"):
        print("it exists")
        return load_keypoints(f"descriptors_saved/{folder}/{img_path}.pkl")
    ...
    key_points1=load_keypoints(f"key_points_saved/{folder}/{img_path}.pkl")
